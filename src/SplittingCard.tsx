...
import { Paper, Typography, List, ListItem, ListItemIcon, ListItemText, TextField, TextFieldProps } from "@mui/material";
...
const SplittingCard = ({ name, values, setValues }: SplittingCardProps) => {
    ...
    const handleAddValue = (event: React.KeyboardEvent<HTMLElement>) => {
        ...
        if (event.key === 'Enter' && inputComponent !== null) {
            ...
            if (typeof input === 'string') {
                const val: number = parseFloat(input.replace(/,/, '.').replace('€', ''));
                if (!isNaN(val)) {
    ...
    return (
        ...
        >
            ...
            <TextField id={name.toLowerCase()} label={'Add ' + name + ' expense'} onKeyUp={handleAddValue} inputRef={inputRef} />
        </Paper>
    );
